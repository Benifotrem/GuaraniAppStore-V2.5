"""
Script to initialize services with DEFINITIVE prices
...
async def init_services():
    ...
    async with AsyncSessionLocal() as db:
        print('Eliminando servicios anteriores...')
        
        # Delete all existing services
        await db.execute(delete(Service))
        await db.commit()
        
        print('Cargando servicios con precios definitivos...')
        
        for service_data in SERVICES_DATA:
            # Create service
            service = Service(**service_data)
            db.add(service)
            print(f'✓ Creado: {service_data["name"]} - Gs. {service_data["price_monthly"]:,}/mes')
        
        await db.commit()
        print(f'\n✅ {len(SERVICES_DATA)} servicios cargados exitosamente!')
